...
const theme = createTheme({
  ...
  palette: { primary: { main: '#4f46e5' }, secondary: { main: '#7c3aed' } },
  components: {
    MuiButton: { styleOverrides: { root: { borderRadius: 12, textTransform: 'none', padding: '12px 24px' } } },
    MuiPaper: { styleOverrides: { root: { borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' } } }
  }
});
...
function parseAmount(amountStr) {
  ...
  const map = {
    ...
    'семьсот': 700, 'восемьсот': 800, 'девятьсот': 900, 'двадцать': 20, 'тридцать': 30, 'сорок': 40,
    'пятьдесят': 50, 'шестьдесят': 60, 'семьдесят': 70, 'восемьдесят': 80, 'девяносто': 90,
    'одиннадцать': 11, 'двенадцать': 12, 'тринадцать': 13, 'четырнадцать': 14, 'пятнадцать': 15,
    'шестнадцать': 16, 'семнадцать': 17, 'восемнадцать': 18, 'девятнадцать': 19, 'десять': 10,
  ...
  const handleBackendAction = useCallback((action) => {
    ...
    switch (action.action_id) {
      case 'add_food': {
        ...
        const normalize = (s) => {
          let res = s.toLowerCase().trim().replace(/^\d+\s*г\s*/i, '');
          if ('ыиаяе'.includes(res.slice(-1))) res = res.slice(0, -1);
          if (res === 'яйц') res = 'яйцо';
        ...
        const found = foodDatabase.find(f => f.name.toLowerCase().includes(normalize(productName)) || normalize(productName).includes(f.name.toLowerCase()));
        if (found) {
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ flexGrow: 1, pb: 8 }}>
        <AppBar position="static" color="primary">
          <Toolbar>
            <Typography variant="h6" sx={{ flexGrow: 1 }}>🥗 Дневник питания</Typography>
